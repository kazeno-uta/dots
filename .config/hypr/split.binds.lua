...
-- Lock screen
hl.bind("SUPER + ALT + L", hl.dsp.exec_cmd("uwsm app -- hyprlock"))
...
hl.bind("SUPER + L", hl.dsp.exec_cmd(fileManager))
...
hl.bind(
	...
	hl.dsp.exec_cmd(
		"sh -c 'choice=\"$("
			.. menu
			.. ' --show drun -b -I -i --define=drun-print_desktop_file=true)"; [ -n "$choice" ] && gtk-launch "$(basename "${choice%.desktop}")"\''
	)
)
hl.bind("SUPER + SHIFT + SPACE", hl.dsp.exec_cmd('xdg-open "$(fd . ~ | ' .. menu .. " --dmenu -i -p 'Search Files')\""))
...
hl.bind("SUPER + t", hl.dsp.exec_cmd("zen-browser"))
...
hl.bind(
	"XF86MonBrightnessUp",
	hl.dsp.exec_cmd("uwsm app -- ~/.config/hypr/scripts/brightness.sh + 5 &"),
...
hl.bind(
	"XF86AudioPlay",
	hl.dsp.exec_cmd("uwsm app -- playerctl play-pause -- the stupid key is called play , but it toggles ")
)
